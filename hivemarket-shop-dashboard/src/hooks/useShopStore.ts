import { getAllShopProductsApi } from "../../src/api/productApi";
import { formatTimeAgo } from "../../src/store/productStore";
import { RecentListingItem } from "../../src/types/products";
import { useCallback, useEffect, useState } from "react";

export function useShopStore(userId: string | null,shopId: string | null) {
  ...
  const fetchProducts = useCallback(async () => {
    try {
      ...
      const data = await getAllShopProductsApi(userId, shopId);
